...
int main(int argc,char **argv){
 ...
 char *p,*tok; for(p=argv[22],tok=strtok(p,",");tok;tok=strtok(NULL,",")){char *eq=strchr(tok,'=');if(!eq){k16_memory_destroy(&m);return 2;}*eq=0;k16_write8(&m,(uint32_t)strtoul(tok,0,0),(uint8_t)strtoul(eq+1,0,0));}\n (void)k16_cpu_step(&c,&m);\n if(!same_cpu(&c,&e)){fprintf(stderr,"%s: CPU mismatch pc=%04x/%04x p=%02x/%02x a=%04x/%04x x=%04x/%04x y=%04x/%04x\\n",argv[1],c.pc,e.pc,c.p,e.p,c.a,e.a,c.x,e.x,c.y,e.y);k16_memory_destroy(&m);return 1;}\n for(p=argv[23],tok=strtok(p,",");tok;tok=strtok(NULL,",")){char *eq=strchr(tok,'=');uint32_t ad;uint8_t want,got;if(!eq){k16_memory_destroy(&m);return 2;}*eq=0;ad=(uint32_t)strtoul(tok,0,0);want=(uint8_t)strtoul(eq+1,0,0);got=k16_read8(&m,ad);if(got!=want){fprintf(stderr,"%s: RAM mismatch %06x=%02x/%02x\\n",argv[1],ad,got,want);k16_memory_destroy(&m);return 1;}}\n k16_memory_destroy(&m);return 0;
}
