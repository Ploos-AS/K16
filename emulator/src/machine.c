...
void k16_machine_reset(k16_machine_t *m){m->ticks=0;k16_vpu_reset(&m->vpu);k16_copper_reset(&m->copper);k16_dma_reset(&m->dma);k16_input_reset(&m->input,&m->memory);k16_uart_reset(&m->uart,&m->memory);k16_ethernet_reset(&m->ethernet,&m->memory);k16_cpu_reset(&m->cpu,&m->memory);}
