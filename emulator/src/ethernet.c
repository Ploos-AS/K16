...
int k16_ethernet_start_rx_dma(k16_ethernet_t*e,uint32_t a){if(!e->rx_ready||e->dma_active)return -1;e->rx_addr=a&K16_ADDRESS_MASK;e->dma_pos=0;e->dma_len=e->rx_len;e->dma_tx=0;e->dma_active=1;return 0;}
int k16_ethernet_start_tx_dma(k16_ethernet_t*e,uint32_t a,uint16_t n){if(n>K16_ETH_FRAME_MAX||e->dma_active)return -1;e->tx_addr=a&K16_ADDRESS_MASK;e->dma_pos=0;e->dma_len=n;e->tx_len=0;e->dma_tx=1;e->dma_active=1;return 0;}
uint8_t k16_ethernet_dma_wants_slot(const k16_ethernet_t*e){return e->dma_active;}
uint8_t k16_ethernet_dma_step(k16_ethernet_t*e,k16_memory_t*m){if(!e->dma_active)return 0;if(e->dma_tx)e->tx[e->dma_pos]=k16_read8(m,e->tx_addr+e->dma_pos);else k16_write8(m,e->rx_addr+e->dma_pos,e->rx[e->dma_pos]);e->dma_pos++;if(e->dma_pos<e->dma_len)return 0;e->dma_active=0;if(e->dma_tx)e->tx_len=e->dma_len;else e->rx_ready=0;sync(e,m);return 1;}
